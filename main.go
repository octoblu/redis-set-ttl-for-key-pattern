...
func main() {
	...
	app.Flags = []cli.Flag{
		cli.StringFlag{
			Name:   "example, e",
			EnvVar: "REDIS_SET_TTL_FOR_KEY_PATTERN_EXAMPLE",
...
func run(context *cli.Context) {
	...
	for {
		...
		debug("redis-set-ttl-for-key-pattern.loop: %v", example)
...
func getOpts(context *cli.Context) string {
	...
	if example == "" {
		...
		if example == "" {
			color.Red("  Missing required flag --example or REDIS_SET_TTL_FOR_KEY_PATTERN_EXAMPLE")
		}
		os.Exit(1)
	}
